...
class DataCleaner:
    def __init__(self, threshold_loss=0.3, threshold_abnormal=0.3, threshold_constant=0.3):
        """
        初始化数据清洗器
        Args:
    ...
    def clean_load_data(self, df, rated_transform_power, rated_power, rated_power_pv, load_col='load'):
        """
        清洗负荷数据
        Args:
            df: 包含负荷数据的DataFrame
        ...
        # Step 2: 检测正常范围外的值
        df = self._handle_abnormal_values(df, load_col, rated_transform_power, rated_power, rated_power_pv)
    ...
    def _handle_missing_data(self, df, load_col):
        ...
        # 判断是否还有空值
        if df[load_col].isna().sum() > 0:
            logger.warning("还有剩余的缺失值，请检查数据质量")
            while df[load_col].isna().sum() > 0:
                df[load_col].fillna(df[load_col].shift(96), inplace=True)
                df[load_col].fillna(df[load_col].shift(-96), inplace=True)
    ...
    def _handle_abnormal_values(self, df, load_col, rated_transform_power, rated_power, rated_power_pv):
        """处理异常值"""
        ...
        if mask.sum() > 0:
            logger.info(f"检测到 {mask.sum()} 个异常值超出正常范围并已处理")
